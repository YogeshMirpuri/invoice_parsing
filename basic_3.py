...
def extraction(file):
    ...
    for i in range(1,100):
        ...
        # x = "description 6?"
        x = "description "+str(i)+"?" 
        desc = p.run(query=x, documents=[Document(content=context)])
   
        if ((desc['answers'][0].score >= 0.6) and (desc['answers'][0].score <= 1) ) and desc['answers'][0].answer is not "":
            ...
            print("NUMBER OF ITEMS IN INVOICE = ", i-1, ".", x, "doesn't exist")
            break
