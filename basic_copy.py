...
def extraction(file):
    ...
    for i in range(1,11):
        ...
        if desc['answers'][0].answer is not None:
...
def result_data(file):
    ...
    # print(data)
    # vendor_master_data = Db.vendor_master_data()
    # item_master_data = Db.item_master_data()
    result={}
    # for i, row in enumerate(vendor_master_data):
    #     #print(row)
    #     if data['Seller name?']==row[1]:
    #         if data['Address?']==row[2]:
    #             result['vendor_code'] = 000000
    
    # for item_num in range(1,11):
    #     for i, row in enumerate(item_master_data):
    #         desc = 'description '+str(item_num)+'?'
    #         #print(desc) 
    #         if data[desc]==row[2]:
    #             prod = 'product_number'+str(item_num)
    #             result[prod] = row[0]
    #             result['product_number'] = 000000
    ...
    result['product_description1']=data['description 1?']
    result['quantity1']=data['Order quantity 1?']
    result['net_price1']=data['Net unit price 1?']
    result['amount1']=data['Amount 1?']
